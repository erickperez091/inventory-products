pipeline {
    ...
    environment {
        ...
        DOCKER_IMAGE = "erickperez091/dev-products-service"
    }
        ...
        stage('Checkout') {
            steps {
                ...
                checkout([$class: 'GitSCM',
                          ...
                          userRemoteConfigs: [[url: 'https://github.com/erickperez091/inventory-products.git']]])
            }
        }
                ...
                nexusArtifactUploader(
                    ...
                    artifacts: [
                        [
                            artifactId: 'products',
                            classifier: '',
                            file: "target/products-${params.VERSION}.jar",
                            type: 'jar'
                        ],
                        [
                            artifactId: 'products',
        ...
        stage('Build Docker Image') {
            steps {
                script {
                    sh """
                        docker build \
                          --build-arg JAR_FILE=target/products-${params.VERSION}.jar \
